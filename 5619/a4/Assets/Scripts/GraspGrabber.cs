    ...
    void Update()
    {
        ...
        if (spindleFlag.GetComponent<spindleFlag_script>().flag)
        {
            ...
            if (grabbedObject)
            {
                ...
                if (Vector3.Distance(otherController.transform.position, this.gameObject.transform.position) >= .825f)
                {
                    grabbedObject.transform.localScale += new Vector3(.005f, .005f, .005f);
                }
                else if (Vector3.Distance(otherController.transform.position, this.gameObject.transform.position) < .06125f)
                {
                    grabbedObject.transform.localScale -= new Vector3(.005f, .005f, .005f);
                }
            }
